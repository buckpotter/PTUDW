<x-layout>
  ...
  <h1 class="header">Chỉnh sửa thông tin chuyến xe</h1>

  <x-form>
    @method('PUT')
    <x-slot name="action">{{ route('trips.update', $trip->IdChuyen) }}</x-slot>
    <x-slot name="header">Chuyến xe {{ $trip->IdChuyen }}</x-slot>
    ...
    <div class="grid grid-cols-2 gap-8 my-6">
      ...
      <x-input-floating-label>
        ...
        <x-slot name="value">{{ $trip->So_xe }}</x-slot>
        Số xe
      </x-input-floating-label>
      ...
      <x-input-floating-label>
        ...
        <x-slot name="value">{{$trip->GiaVe}}</x-slot>
        Giá vé
      </x-input-floating-label>
    </div>
